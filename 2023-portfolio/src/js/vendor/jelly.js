...
import { getColor, getVar } from "../utils";

export default class JellyEffect {
    constructor(options) {
        this.jellys = [];
        this.els = document.querySelectorAll('[data-jelly]');
...
export class Jelly {
    ...
    loop() {
        ...
        const diff = Math.abs(y - this.y);
        const skew = Math.min(Math.max(this.options.min, diff * this.options.intensity), this.options.max);

        if (!this.paused) {
            gsap.set(this.el, { scale: 1 - skew, force3D: true });
            this.animationPaused = false;
        } else {
            if (!this.animationPaused) {
                gsap.to(this.el, { scale: 1, force3D: true, duration: this.options.speed });
